...
Name: William Yang
Date: 12/6/21
Description: This assignment focuses on using singly linked lists with a Polynomial
class. Each Polynomial has its terms represented by the nodes of the singly linked
lists, which are used for other operations (in which this assignment emphasizes on),
like overloaded operators (+, +=, ==, !=), copy control, and some miscellaneous
methods like evaluate, which evaluates a polynomial fora  given x value, or
cleanup, which deletes all leading 0 terms. This assignment also uses separate
compilation, meaning the polynomial has its own header file and cpp file, in which
the provided code uses via #include.
*/
...
int main() {
    ...
    // temp = p2 + p3;
    // cout << "temp: " << temp << endl;
    cout << "p2 + p3: " << (p2+p3) << endl; // 7
    cout << "p2 + p4: " << (p2+p4) << endl; // 5x^4 + 4x^3 + 3x^2 + 3x + 3
    cout << "p4 + p2: " << (p4+p2) << endl; // 5x^4 + 4x^3 + 3x^2 + 3x + 3
